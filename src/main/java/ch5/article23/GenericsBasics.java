 ...
 * коллекции, и, если кто-то вставит объект неправильного типа, то программа рухнет. С помощью же дженериков, мы даем
 ...
 * Кое-что интересное представленно ниже в методе unsafeAdd. Здесь в аргументе функции исполбзуется необработанный тип
 ...
 *      Set<?> - это тип подстановки, который может содержать только объекты некоторого неизвестного типа
...
public class GenericsBasics {
    ...
    static int numElementInCommon(Set<?> s1, Set<?> s2, List<?> l1, List<Object> l2) {
        l1.add(null);
        l2.add("qwe");
        l2.add(1488);
        int result = 0;
