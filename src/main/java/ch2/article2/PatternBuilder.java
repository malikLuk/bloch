/**
 *
 *                                              Исползовать шаблон Builder, когда
 *                                         приходится иметь дело с большим количеством
 *                                                   параметров конструктора.
 * У конструткоров и статических методов есть одно общее ограничение: они плохо масштабируют большое количество необязательных
 ...
 * также ряд необязательных параметров - общее содержание жиров, содержание насыщенных жиров, холестерина, натрия и т д.
 ...
 * не все поля.
 * К счастью - есть решение. Оно является одной из форм шаблона Строитель. Вместо непосредственного создания желаемого
 ...
 * build() для генерации объекта, который будет являться неизменным. Строитель является статическим внутренним классом
